...
if(isset($_POST['submit'])) {
	...
	if($_FILES['image']['name'] == "") {
        ...
        if($coption == 1) {
                $sql = $conn->Prepare('UPDATE newser SET main = ?, idblog = ?, btexty = ?, bhelper = ?, brief = ?, images = ?, bamess = ? WHERE  `blogid` = ? and buserid = ?');
                if($conn->Execute($sql,array($main,$gruppe,$btextyx,$helper,$briefyx,$new_image,$bamessy,$id,$shouter)) === false) {
		...
		print '<br /><div id="error">error inserting[1]: '.$conn->ErrorMsg().'</div><br />';
                ...
                $sql3 = $conn->Prepare('UPDATE newser SET main = ?, idblog = ?, btexty = ?, bhelper = ?, brief = ?, bamess = ? WHERE `blogid` = ? and buserid = ?');
                if($conn->Execute($sql3,array($main,$gruppe,$btextyx,$helper,$briefyx,$bamessy,$id,$shouter)) === false) {
	        print '<br /><div id="error">error inserting[1]: '.$conn->ErrorMsg().'</div><br />';
	}
                $bamessy = htmlspecialchars_decode($bamessy);
                $bamessy = strip_tags($bamessy);
...
<?php } ?>
<form method="post" id="inrform" action="edit.php?id=<?php echo $arecordSet->fields['blogid'] ?>" enctype="multipart/form-data" method="post">
...
if($main == 2){
?>
<input type="radio" name="main" id="2" value="2" checked="checked" />&nbsp;Yes [<?php echo $lang['SPONSOR']; ?>]
<?php
}
?>
<br /><br />
