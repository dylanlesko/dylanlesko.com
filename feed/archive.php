...
$brecordSet = &$conn->Execute('SELECT btexty, bdate, COUNT(*) AS total, YEAR(bdate) AS year, MONTH(bdate) AS month FROM newser WHERE main = '.$conn->qstr('0').' or main = '.$conn->qstr('2').' GROUP BY month, year order by month, year');
if(!$brecordSet)
	print $conn->ErrorMsg();
else
...
$smarty->assign('data', $bval);
$smarty->assign('tr', array('bgcolor="#FCFCFC"','bgcolor="#FAFAFA"') );
$smarty->display('archive.php');
